...
class Aviation(commands.Cog):
    ...
    async def metar(self, ctx, station_id: str):
        ...
        try:
            ...
            t = metar_time['dt']
            body += f"**Observed at**: {t[: t.find('T')]} {t[t.find('T') + 1 : t.find('T') + 2]}{t[t.find('T') + 3 : t.find('T') + 4]}{t[t.find('T') + 4 : t.find('T') + 6]}Z\n"
            ...
            body += f"**Time at station**: {ts[: ts.find('T')]} {ts[ts.find('T') + 1 : ts.find('T') + 2]}{ts[ts.find('T') + 3 : ts.find('T') + 4]}{ts[ts.find('T') + 4 : ts.find('T') + 6]}Z\n"
